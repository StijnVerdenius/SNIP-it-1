...
This repository contains the implementation of the paper 'Pruning via Iterative Ranking of Sensitivity Statistics' _(currently anonymous for review)_.
...
- Structured (node) pruning before training (SNAPit)
- Unstructured (weight) pruning before training (SNIPit)
- Unstructured (weight) pruning during training (SNIPitDuring)
...
If you mean to run the 'Imagenette' dataset: download that from [here](https://github.com/fastai/imagenette) and unpack in `/gitignored/data/`, then replace CIFAR10 with IMAGENETTE in commands below to run. Additional datasets can be added in a similar way (Imagewoof, tiny-imagenet, etc.)
